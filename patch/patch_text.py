...
def patchText(collection, from_lesson, to_lesson, sleep=0):
    texts_folder = "/Users/rafa/Desktop/lingq/post/split"
    texts = readGreekNumerals(texts_folder)

    for text_filename, lesson in list(zip(texts[1:], collection['lessons'][2:]))[from_lesson-1:to_lesson]:
        lesson = requests.get(lesson['url'], headers=header).json()
        lesson_id = lesson["id"]
...
def main():
    ...
    collection = requests.get(f"{API_URL_V2}{language_code}/collections/{pk}", headers=header).json()
